...
export function ExperienceTimeline({ experiences }: ExperienceTimelineProps) {
  return (
    <div className="relative space-y-8">
      {/* Timeline line */}
      <div className="absolute left-4 top-0 h-full w-0.5 bg-border-strong sm:left-1/2 sm:-translate-x-0.5" />
      ...
      {experiences.map((exp, index) => (
        ...
        >
          {/* Timeline dot */}
          <div
            className={`absolute left-3 top-1 h-3 w-3 rounded-full border-2 border-background bg-foreground sm:left-auto ${
              index % 2 === 0 ? "sm:-right-1.5" : "sm:-left-1.5"
            }`}
          />

          <div className="rounded-2xl border border-border bg-surface p-5 shadow-[var(--shadow-glass)] backdrop-blur-xl">
