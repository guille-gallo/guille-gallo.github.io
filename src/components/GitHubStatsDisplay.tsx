    ...
    <div className="grid grid-cols-1 gap-4 sm:max-w-xs sm:grid-cols-1 sm:mx-auto">
      {statItems.map((item, index) => (
        <motion.div
          ...
          className="flex flex-col items-center rounded-2xl border border-border bg-surface p-4 text-center shadow-[var(--shadow-glass)]"
        >
          <item.icon className="mb-2 h-5 w-5 text-muted" />
          <span className="text-2xl font-medium text-foreground-strong">
            {item.value}
          </span>
