...
export function ProjectCard({ project, index = 0 }: ProjectCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex h-full flex-col rounded-2xl border border-border bg-surface-2 p-5 transition-colors hover:border-border-strong"
    >
      {/* Header */}
      <div className="mb-2 flex items-start justify-between">
        ...
        {project.vercelUrl && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 rounded-md border border-border bg-surface px-2 py-0.5 text-[11px] font-medium text-foreground transition-colors hover:border-accent hover:text-accent"
          >
      ...
      <div className="mt-auto flex items-start gap-2 pt-2">
        ...
        {githubData?.html_url && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="ml-auto shrink-0 self-end rounded-md p-1 text-muted transition-colors hover:bg-surface hover:text-accent"
